...
class M_transaksi extends CI_Model {
	 public function simpan_cart_db()
	 {
	 	for($i=0; $i<count($this->cart->contents()); $i++){
				...
				$stok = $this->db->where('id_produk', $this->input->post('id_produk')[$i])
								 ->get('produk')
								 ->row()
								 ->stok;
	 ...
	 {
	 	return $this->db->where('id_transaksi', $id_transaksi)
	 					->join('produk','produk.id_produk=nota.id_produk')
	...
	public function cek($id_produk){
		...
		$cek_stok = $this->db->where('id_produk', $id_produk)->get('produk')->row()->stok;
